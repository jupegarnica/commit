# Commit

Write commit messages powered by AI

## Requirements

This script requires Deno to run. If you don't have Deno installed, you can
install it by following the instructions on the
[official Deno website](https://deno.land/).

It needs git to be installed and configured in your system. You can install git
by following the instructions on the
[official git website](https://git-scm.com/).

You will need an API key for your chosen AI provider. The default provider is
OpenAI — sign up and get an API key on the
[OpenAI website](https://platform.openai.com/).

# Install

```sh
deno install -Agrfn commit  jsr:@garn/commit
```

## Usage

```sh
commit [OPTIONS]
```

- `--add`: Runs `git add .` before creating the commit message.
- `--push`: Runs `git push` after creating the commit.
- `--amend`: Runs `git commit --amend` instead of `git commit`.
- `--commits-to-learn <number>`: Specifies the number of commits to learn from.
  The default is 10.
- `--skip-edit`: Skips the interactive preview and editing of the commit message
  before creating the commit.
- `--no-commit`: Skips the creation of the commit. Just prints the commit
  message.
- `--provider <provider>`, `-p`: Specifies the AI provider to use. Valid
  options: `openai` (default), `google`, `ollama`, `ollama-cloud`, `anthropic`.
  Each provider has its own default model and environment variable for the API
  key.
- `--model <model>`: Specifies the model to use. Defaults to the provider's
  default model.
- `--config`: Prompts for the default options and saves them.
- `--api-key <apiKey>`: Specifies the API key to use. Overrides the provider's
  environment variable (`OPENAI_API_KEY`, `GEMINI_API_KEY`, `ANTHROPIC_API_KEY`,
  `OLLAMA_API_KEY`, etc.).
- `--max-words <maxWords>`: Specifies the maximum number of words to send to the
  API. The default is 6000. Useful to avoid extra charges.
- `--co-author <pattern>`: Appends a signature (trailer) to the commit message.
  Placeholders: `{model}` (resolved model id) and `{email}` (co-author email for
  the provider; prompted and saved on first use). Example:
  `--co-author "Co-Authored-By: {model} <{email}>"`. Can also be saved with
  `--config` as the global `co-author` setting so it applies to every commit
  (leave empty there to disable it). See
  [Co-author signature](#co-author-signature).
- `--co-author-email <email>`: Overrides the co-author email for this run
  (resolves the `{email}` placeholder). Takes precedence over the saved provider
  `co-author-email` config and skips the first-use prompt.
- `--base-URL <baseURL>`: Specifies a custom base URL for the provider API.
  Overrides the provider's default. For `ollama`, can also be set via
  `OLLAMA_BASE_URL`. Required for the `ollama` provider if not already saved.
- `--debug`: Enables debug mode, which will print additional information to the
  console.
- `--help`: Prints the help message.
- `--version`: Prints the version number.

Any extra options not recognized by this CLI are forwarded to `git commit`. If
an option conflicts with this CLI, pass it after `--`.

### Providers

| Provider       | Default model              | API key env var     | Base URL env var  |
| -------------- | -------------------------- | ------------------- | ----------------- |
| `openai`       | `gpt-5-nano`               | `OPENAI_API_KEY`    |                   |
| `google`       | `gemini-2.0-flash`         | `GEMINI_API_KEY`    |                   |
| `ollama`       | `llama3`                   | _(not required)_    | `OLLAMA_BASE_URL` |
| `ollama-cloud` | `kimi-k2.5:cloud`          | `OLLAMA_API_KEY`    |                   |
| `anthropic`    | `claude-sonnet-4-20250514` | `ANTHROPIC_API_KEY` |                   |

The `ollama` provider requires a base URL (your local Ollama instance). It is
read from `OLLAMA_BASE_URL`, `--base-URL`, a saved `--config` value, or prompted
interactively if none are found.

The `ollama-cloud` provider requires an API key read from `OLLAMA_API_KEY`,
`--api-key`, or a saved `--config` value.

When `--provider` is passed explicitly on the CLI, any saved `model` config
value is ignored to avoid cross-provider mismatches. The saved `base-URL` is
still used for providers that require it (e.g. `ollama`).

### Environment Variables

- `OPENAI_API_KEY`: API key for the OpenAI provider.
- `GEMINI_API_KEY`: API key for the Google provider.
- `ANTHROPIC_API_KEY`: API key for the Anthropic provider.
- `OLLAMA_API_KEY`: API key for the `ollama-cloud` provider.
- `OLLAMA_BASE_URL`: Base URL for the local `ollama` provider (e.g.
  `http://localhost:11434/v1`).

## Example

Here's an example of how you can use `commit`:

```sh
OPENAI_API_KEY=your_api_key_here commit --add --push
```

Using Anthropic Claude as the provider:

```sh
ANTHROPIC_API_KEY=your_api_key_here commit --provider anthropic
```

Using Google:

```sh
GEMINI_API_KEY=your_api_key_here commit --provider google --add --push
```

Using a local Ollama instance:

```sh
OLLAMA_BASE_URL=http://localhost:11434/v1 commit --provider ollama
```

Using Ollama Cloud:

```sh
OLLAMA_API_KEY=your_api_key_here commit --provider ollama-cloud
```

Passing extra `git commit` options:

```sh
OPENAI_API_KEY=your_api_key_here commit --add --push --no-verify
```

If there is a conflict, use `--`:

```sh
OPENAI_API_KEY=your_api_key_here commit -- --no-verify
```

This will add all changes, create a commit with a message generated by the AI
provider, and push the changes to the current branch.

### Co-author signature

You can automatically append a signature (trailer) to every commit message. It
is configured with two settings:

- `co-author` (global): the template of the signature. Supports the `{model}`
  placeholder (the model id actually used) and `{email}` (the co-author email
  for the current provider). Empty means disabled.
- `co-author-email` (per provider): the email used to resolve `{email}` for that
  provider (e.g. `noreply@anthropic.com` for `anthropic`).

Both can be set with `--config`, or the template can be passed just for one run
with `--co-author` (and the email with `--co-author-email`, which overrides the
saved one for that run without saving it):

```sh
OPENAI_API_KEY=your_api_key_here commit --add \
  --co-author "Co-Authored-By: {model} <{email}>"
```

The resulting commit message looks like:

```
feat: add oauth flow

Co-Authored-By: gpt-5-nano <noreply@openai.com>
```

The first time a template with `{email}` runs for a provider and that provider
has no email saved, you are prompted for it (suggested:
`noreply@<provider>.com`) and it is saved in the config for the next runs. If
the prompt is answered empty, that run has no signature and nothing is saved.

Once saved with `--config`, the settings look like:

```json
{
  "co-author": "Co-Authored-By: {model} <{email}>",
  "providers": {
    "anthropic": { "co-author-email": "noreply@anthropic.com", "...": "..." }
  }
}
```

The signature is appended after the message is generated, so it is visible and
editable in the interactive preview (and it is also applied with `--skip-edit`
and `--no-commit`). If the message already contains the signature (e.g. when
amending), it is not duplicated.

The email for `{email}` is resolved as: `--co-author-email` flag > saved
provider `co-author-email` > first-use prompt. Example:

```sh
OPENAI_API_KEY=your_api_key_here commit --add \
  --co-author "Co-Authored-By: {model} <{email}>" \
  --co-author-email me@example.com
```

This produces `Co-Authored-By: gpt-5-nano <me@example.com>` without changing the
saved config.
